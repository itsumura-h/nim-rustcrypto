...
RustCrypto を Nim から安全に呼び出すための FFI ラッパーです。現時点では SHA-256、HMAC-SHA256、HKDF-SHA256、PBKDF2-HMAC-SHA256、scrypt、Argon2id、PHC 文字列の検証・正規化、AES-256-GCM、AES-256-GCM-SIV、BLAKE2b-512、BLAKE2s-256、secp256k1 公開鍵導出、ECDSA 署名・検証、SHA-256 / SHA3-256 / Keccak-256 を使う secp256k1 ECDSA 署名・検証、`nim_rustcrypto/algorithm/schnorr.nim` から使う secp256k1 Schnorr の低水準公開鍵導出・署名・検証、Ed25519 の公開鍵導出・署名・検証、PKCS#8/SPKI 変換と PEM 変換を提供しています。Schnorr は ECDSA とは別形式で、公開鍵と署名は互換ではありません。
